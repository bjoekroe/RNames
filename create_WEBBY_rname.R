###############################################################################################################################
###### This file creates W_TS.csv, a list with all Rnames names binned within Webby et al. (2004) time slices  ###
###### same as http://rnames.luomus.fi/index.php/search-time-slices #############################################
###############################################################################################################################
...
bio.names <- dbGetQuery(rn.con,"
                        ...
                        and r.qualifier_name_2 = 'TimeSlice_Webby'
...
if (length(bio.names.unique) > 0){
  for (i in 1:NROW(bio.names.unique)){
    result <- compromise.selector(ts_used = 'W', subset( bio.names, name_1 == bio.names.unique[i]))
...
repeat
{
  bio.names <- dbGetQuery(rn.con,"
                          ...
                          where r.strat_qualifier_1 = 'Biostratigraphy'
                          and r.qualifier_name_1 != 'TimeSlice_Webby'
...
if (length(bio.names.unique) > 0){
  for (i in 1:NROW(bio.names.unique)){
    result <- youngest.selector(ts_used = 'W', subset( bio.names, name_1 == bio.names.unique[i]))
...
rn.con <- dbConnect(m.drv, dbname = db.db, username = db.user, password = db.pw, host = db.url)
dbGetQuery(rn.con," SET NAMES utf8")
dbGetQuery(rn.con,"delete from opinions_webby")
dbWriteTable(rn.con, "opinions_webby", W_TS, row.names=FALSE, append=TRUE)
dbGetQuery(rn.con,"delete from opinions_webby_reference")
dbGetQuery(rn.con,re_query_W)
dbDisconnect(rn.con)

write.csv(W_TS, file = "W_TS.csv")
